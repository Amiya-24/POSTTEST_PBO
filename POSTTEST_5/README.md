...
- **`POSTTEST_4` (Polymorphism):** Penerapan *Method Overriding* (diskon spesifik per alat) dan *Method Overloading* (kalkulasi pajak pada struk).
- **`POSTTEST_5` (Abstraction & Interface):** 
    - **Abstract Class & Method:** Mengubah `AlatBerat` menjadi *Abstract Class* yang bertindak sebagai *template* kerangka, serta menjadikan `hitungBiayaAlat()` sebagai *Abstract Method* yang wajib di-*override* oleh subclass.
    - **Interface:** Pembuatan antarmuka `LayananSewa` sebagai "kontrak" yang mendefinisikan method `hitungTotalBiaya()` dan `cetakStruk()`. Interface ini kemudian diimplementasikan sepenuhnya oleh class `Penyewaan`.
